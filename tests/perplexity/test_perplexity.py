...
class TestPerplexity:
    ...
    def test_pad_tensors_return_correct_shape(self):
        ...
        attn_mask = [torch.ones(2, dtype=torch.int), torch.ones(5, dtype=torch.int), torch.ones(3, dtype=torch.int)]
        ...
        padded_logits, padded_labels, padded_attn_mask = perp._pad_tensors(logits, labels, attn_mask)
    ...
    def test_threshold_filtering(self):
        ...
        threshold = 0.2
        (filtered_logits, filtered_labels,
         filtered_attn_mask), probs_collection, tt, ft, longest_sequences = perp._filter_on_threshold(
            logits, labels, attn_mask, threshold)
